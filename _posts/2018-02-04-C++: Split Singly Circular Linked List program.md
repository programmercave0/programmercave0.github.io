---
layout: post
title: "C++: Split Singly Circular Linked List program"
tags:  [C++, Algorithm, Linked List, Data Structures]
date: 2018-02-04
---

Given a Singly Circular Linked List, we have to split it into two equal halves. 

If the number of nodes present in the given list is odd then the first half will contain one extra node. 

We have one *slow pointer* which advances by one node and one *fast pointer* which advances by second node. 

First we will find the middle node of the list. Then split the list and the node next to the middle node will be the head of second half and middle node will be the last node of first half. 

![Split Circular LInked List]({{ site.url }}/assets/SplitCLL1.png)
...
![Output]({{ site.url }}/assets/SplitCLLOut.png)
